...
pageType: article
sidebarTitle: "Deductive and Inductive Interfaces"
tags: [architecture, domain-driven-design, software-design, domain-modeling, user-interface, command-query-separation]
---
...
This command traverses the system with its semantics intact. The backend has nothing left to deduce: it directly receives a clear order that it can [validate](/en/advanced-software-engineering/type-design/parse-dont-validate), execute, and log.
...
5. The [aggregate](/en/advanced-software-engineering/type-design/state-machines-and-aggregates-in-ddd) implements an `approve()` method
...
- **On the write side**: each command can carry [exactly the data necessary](/en/advanced-software-engineering/type-design/make-illegal-states-unrepresentable) for its validation, no more, no less
