...
A [pure function](/en/advanced-software-engineering/total-and-partial-functions-the-type-promise), which depends only on its arguments and produces only its return value, is trivially testable: you provide inputs, you verify outputs.
...
[TDD](/en/advanced-software-engineering/tdd-a-design-discipline-not-a-testing-one) inverts the usual causality: rather than designing then wondering how to test, we start from the test and let testability guide the design.
...
Tests then become not an additional burden but a natural consequence of sound design. Conversely, truly well-designed code invites testing: entry points are obvious, behaviors are observable, [edge cases are accessible](/en/advanced-software-engineering/property-based-testing-power-and-limits).
