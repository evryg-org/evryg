...
An ADT is constructed by composing [two fundamental operations](/en/type-algebra-products-and-sums):
...
Rather than an `Order` with optional fields whose validity depends on a `status` field, we model [an explicit state machine](/en/state-machines-and-aggregates-in-ddd):
...
The invalid state "shipped order without shipping date" becomes inexpressible: not forbidden by runtime validation, but structurally impossible. This is the essence of [Make Illegal States Unrepresentable](/en/make-illegal-states-unrepresentable): ADTs provide the vocabulary for the type system to capture business rules, not just primitive types.
