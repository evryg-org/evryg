...
This correspondence allows applying the [Make Illegal States Unrepresentable](/en/make-illegal-states-unrepresentable) principle with fine granularity.

Rather than a monolithic aggregate with optional fields whose presence depends on the current state, we can model each state as a distinct variant of a [sum type](/en/type-algebra-products-and-sums), carrying exactly the relevant data:
