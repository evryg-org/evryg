...
Un ADT se construit par composition de [deux opérations fondamentales](/fr/algebre-des-types-produits-et-sommes) :
...
Plutôt qu'un `Order` avec des champs optionnels dont la validité dépend d'un champ `status`, on modélise [une machine à état explicite](/fr/machines-a-etat-et-aggregats-en-ddd) :
...
L'état invalide « commande expédiée sans date d'expédition » devient inexprimable : non pas interdit par une validation runtime, mais structurellement impossible. C'est l'essence de [Make Illegal States Unrepresentable](/fr/make-illegal-states-unrepresentable) : les ADT fournissent le vocabulaire pour que le système de types capture les règles métier, pas seulement les types primitifs.
