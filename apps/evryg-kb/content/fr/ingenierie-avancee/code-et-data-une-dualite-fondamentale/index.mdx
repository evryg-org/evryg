...
Un programme source n'est, après parsing, qu'un [arbre de syntaxe abstraite](/fr/les-types-de-donnees-algebriques-precision-et-expressivite) : une structure de données comme une autre, avec des nœuds, des branches, des feuilles.

Lisp a rendu cette équivalence explicite dès ses origines : le code est une liste, les listes sont des données, et `eval` [transforme une donnée en comportement exécutable](/fr/description-et-interpretation-le-programme-comme-valeur). Les macros, les métaprogrammes, les [compilateurs](/fr/encodages-initial-et-final-deux-philosophies-dembedding) : tous manipulent le code comme une donnée que l'on peut inspecter, transformer, générer.
