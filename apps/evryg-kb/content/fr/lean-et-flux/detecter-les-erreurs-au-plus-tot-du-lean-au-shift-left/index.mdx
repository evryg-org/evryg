...
- Oublier de gérer un cas dans un [pattern matching](/fr/les-types-de-donnees-algebriques-precision-et-expressivite) ? Erreur de compilation.

Ces garde-fous ne détectent pas les erreurs après coup ; ils les empêchent de naître. Comme le gabarit qui ne permet d'insérer une pièce que dans la bonne orientation, le système de types contraint l'espace des programmes possibles à ceux qui [respectent certains invariants](/fr/make-illegal-states-unrepresentable).
...
Investir dans la détection précoce, c'est investir dans la [testabilité](/fr/pas-de-test-sans-testabilite-le-prerequis-oublie), la modularité, la qualité du typage : des propriétés qui améliorent le design bien au-delà de leur utilité pour les tests.
